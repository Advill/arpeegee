...
public class Map {
	...
	public Map() {
		...
		for (int i = 0; i <= 46; i++) {
			...
			// ensures x is not 10 larger than y, and that y is not 10 larger
			// than x
			if (locationsx.size() - 10 >= locationsy.size()
					|| randomgenerator(0, 1) == 1 && !(locationsy.size() - 10 >= locationsx.size())) {
	...
	public void printMap(Player P) {
		// if it falls within the bounds such that i dont have to calculate
		// where the player is.
